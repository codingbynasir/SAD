...
from .models import FeedbackReply, ratings, has_test, diagnostic_test, hospital, package, package_item, feedback
...
from .forms import has_testModel, feedbackForm, RatingForm, authorizeTestAdd, authorizePackageAdd, \
    authorizePackageItemAdd, AuthorTestOrderUpdateForm, AuthorPackageOrderUpdateForm, FeedbackReplyForm
...
class singleHospital(View):
    ...
    def get(self, request, id):
        ...
        feedbacks = feedback.objects.filter(hospital=id, aprove=True)
        replyForm = FeedbackReplyForm
        reply = FeedbackReply.objects.filter(feedbackTo__hospital__name__id=medical.id)
        context = {"hospital": medical, "tests": test, "package": pack, "form": self.form, "feedbacks": feedbacks,
                   "reply": reply, "replyForm": replyForm}
        return render(request, self.template_name, context)
    ...
    def post(self, request, id):
        ...
        if 'replyBtn' in request.POST:
            replyForm = FeedbackReplyForm(request.POST)
            if replyForm.is_valid():
                replyForm.save()
            return redirect('medical:hospital_details', id=m.id)
        else:
            raise Http404("Sorry reply is not saved")
        ...
        if request.user.is_authenticated:
            if request.session['hospital'] is not None:
                if request.session['hospital'] in request.user.username:
                    ...
                    total_order = Test_order.objects.filter(test__hospital__name=request.user.id,
